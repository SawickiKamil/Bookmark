import * as React from 'react'
import { useImageStyles } from './styles'
import classnames from 'classnames'

interface IIllustrationProps extends JSX.IntrinsicAttributes {
  width?: string
  height?: string
  className?: string
  fill?: string
  stroke?: string
}
const Illustration2 = ({ className, fill, width, height }: IIllustrationProps) => {
  const classes = useImageStyles()
  return (
    <div className={classnames(className, classes.iconWrapper)}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        width={width ? width : '478'}
        height={height ? height : '416'}
      >
        <defs>
          <rect id="a" width="350" height="346" rx="8"></rect>
          <rect id="d" width="200" height="255" rx="8"></rect>
          <filter id="c" width="117.5%" height="113.7%" x="-8.8%" y="-4.9%" filterUnits="objectBoundingBox">
            <feOffset dy="5" in="SourceAlpha" result="shadowOffsetOuter1"></feOffset>
            <feGaussianBlur in="shadowOffsetOuter1" result="shadowBlurOuter1" stdDeviation="5"></feGaussianBlur>
            <feColorMatrix
              in="shadowBlurOuter1"
              values="0 0 0 0 0.285332669 0 0 0 0 0.363695184 0 0 0 0 0.811480978 0 0 0 0.0830692745 0"
            ></feColorMatrix>
          </filter>
          <filter id="e" width="139.5%" height="131%" x="-19.8%" y="-13.5%" filterUnits="objectBoundingBox">
            <feOffset dy="5" in="SourceAlpha" result="shadowOffsetOuter1"></feOffset>
            <feGaussianBlur in="shadowOffsetOuter1" result="shadowBlurOuter1" stdDeviation="5"></feGaussianBlur>
            <feColorMatrix
              in="shadowBlurOuter1"
              result="shadowMatrixOuter1"
              values="0 0 0 0 0.285332669 0 0 0 0 0.363695184 0 0 0 0 0.811480978 0 0 0 0.0830692745 0"
            ></feColorMatrix>
            <feMerge>
              <feMergeNode in="shadowMatrixOuter1"></feMergeNode>
              <feMergeNode in="SourceGraphic"></feMergeNode>
            </feMerge>
          </filter>
          <rect id="g" width="200" height="255" rx="8"></rect>
          <filter id="f" width="117.5%" height="113.7%" x="-8.8%" y="-4.9%" filterUnits="objectBoundingBox">
            <feOffset dy="5" in="SourceAlpha" result="shadowOffsetOuter1"></feOffset>
            <feGaussianBlur in="shadowOffsetOuter1" result="shadowBlurOuter1" stdDeviation="5"></feGaussianBlur>
            <feColorMatrix
              in="shadowBlurOuter1"
              values="0 0 0 0 0.285332669 0 0 0 0 0.363695184 0 0 0 0 0.811480978 0 0 0 0.0830692745 0"
            ></feColorMatrix>
          </filter>
        </defs>
        <g fill="none" fillRule="evenodd">
          <mask id="b" fill="#fff">
            <use xlinkHref="#a"></use>
          </mask>
          <use fill="#E8EAF6" fillRule="nonzero" xlinkHref="#a"></use>
          <g mask="url(#b)">
            <path fill="#242A45" d="M-3.53.197h547v59h-547z"></path>
            <g fill="#FFF" transform="translate(22.47 14.197)">
              <circle cx="16" cy="16" r="16" opacity="0.245"></circle>
              <g transform="translate(47 6)">
                <rect width="120" height="6" opacity="0.75" rx="3"></rect>
                <rect width="64" height="6" y="14" opacity="0.25" rx="3"></rect>
              </g>
            </g>
          </g>
          <g transform="translate(27 138)">
            <use fill="#000" filter="url(#c)" xlinkHref="#d"></use>
            <use fill="#FFF" xlinkHref="#d"></use>
            <g transform="translate(23 29)">
              <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
              <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
              <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              <g transform="translate(0 56)">
                <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
              <g transform="translate(0 112)">
                <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
              <g transform="translate(0 168)">
                <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
            </g>
          </g>
          <g filter="url(#e)" transform="translate(251 138)">
            <use fill="#000" filter="url(#f)" xlinkHref="#g"></use>
            <use fill="#FFF" xlinkHref="#g"></use>
            <g transform="translate(23 29)">
              <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
              <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
              <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              <g transform="translate(0 56)">
                <circle cx="16" cy="16" r="16" fill="#FA5959"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
              <g transform="translate(0 112)">
                <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
              <g transform="translate(0 168)">
                <circle cx="16" cy="16" r="16" fill="#5267DF"></circle>
                <rect width="101.317" height="6" x="45.683" y="6" fill="#242A45" opacity="0.5" rx="3"></rect>
                <rect width="54.036" height="6" x="45.683" y="20" fill="#242A45" opacity="0.25" rx="3"></rect>
              </g>
            </g>
          </g>
          <rect width="237" height="34" x="27" y="85" fill="#FFF" rx="8"></rect>
          <g stroke="#5267DF" strokeWidth="20" transform="translate(305 102)">
            <circle cx="67" cy="67" r="67" fill="#495DCF" fillOpacity="0.2"></circle>
            <path strokeLinecap="round" strokeLinejoin="round" d="M114 115l49 49"></path>
          </g>
        </g>
      </svg>
    </div>
  )
}
export default Illustration2
